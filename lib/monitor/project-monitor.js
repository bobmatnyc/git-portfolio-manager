...
class ProjectMonitor {
  ...
  async performScan() {
    ...
    try {
      ...
      this.sendMessage("health_update", {
        ...
        hasTrackdown: scanData.filesystem?.hasTrackdown || false,
      });
  ...
  async analyzeFileSystem() {
    ...
    try {
      ...
      // Perform toolchain detection
      await this.detectToolchain(fsData);
    } catch (error) {
  ...
  /**
   * Detect project toolchain and dependencies
   */
  async detectToolchain(fsData) {
    try {
      // Check for config files that indicate project type
      const configFiles = [];
      const checkFiles = [
      ...
      fsData.configFiles = configFiles;

      // Detect project type based on config files
      if (configFiles.includes('pyproject.toml') || configFiles.includes('requirements.txt') || configFiles.includes('setup.py')) {
        fsData.projectType = 'python';
      } else if (configFiles.includes('package.json')) {
        fsData.projectType = 'node';
      } else if (configFiles.includes('go.mod')) {
        fsData.projectType = 'go';
      } else if (configFiles.includes('Cargo.toml')) {
        fsData.projectType = 'rust';
      } else if (configFiles.includes('pom.xml') || configFiles.includes('build.gradle')) {
        fsData.projectType = 'java';
      } else if (configFiles.includes('composer.json')) {
        fsData.projectType = 'php';
      } else if (configFiles.includes('Gemfile')) {
        fsData.projectType = 'ruby';
      }

      // Detect dependencies and frameworks
      const dependencies = [];
      
      // Check for common directories that indicate dependencies
      if (fs.existsSync(path.join(this.projectPath, 'node_modules'))) {
        dependencies.push('node_modules');
      }
      if (fs.existsSync(path.join(this.projectPath, 'venv')) || fs.existsSync(path.join(this.projectPath, '.venv'))) {
        dependencies.push('python_venv');
      }
      if (fs.existsSync(path.join(this.projectPath, '__pycache__'))) {
        dependencies.push('python_cache');
      }
      ...
      if (configFiles.includes('package.json')) {
        try {
          ...
          if (allDeps.react) dependencies.push('react');
          if (allDeps.next) dependencies.push('next');
          if (allDeps.vue) dependencies.push('vue');
          if (allDeps.angular) dependencies.push('angular');
          if (allDeps.express) dependencies.push('express');
          if (allDeps.typescript) dependencies.push('typescript');
        } catch (error) {
          this.log(`⚠️ Could not parse package.json: ${error.message}`, 'debug');
        }
      }
      ...
      if (configFiles.includes('pyproject.toml')) {
        try {
          ...
          // Simple parsing for common frameworks (basic regex approach)
          if (content.includes('django')) dependencies.push('django');
          if (content.includes('flask')) dependencies.push('flask');
          if (content.includes('fastapi')) dependencies.push('fastapi');
          if (content.includes('pytest')) dependencies.push('pytest');
        } catch (error) {
          this.log(`⚠️ Could not parse pyproject.toml: ${error.message}`, 'debug');
        }
      }

      fsData.dependencies = dependencies;
      
      this.log(`🔍 Detected toolchain: ${fsData.projectType}, dependencies: ${dependencies.join(', ')}`, 'debug');
    } catch (error) {
      this.log(`⚠️ Toolchain detection error: ${error.message}`, 'warn');
    }
  }
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          if (
            ...
            fileCount = await this.countRecentFiles(path.join(dir, dirName), since, fileCount);
          }
        } else {
