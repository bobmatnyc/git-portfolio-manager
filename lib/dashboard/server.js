...
const TrackDownManager = require("../trackdown/trackdown-manager");

class DashboardServer {
  constructor(options = {}) {
    this.port = options.port || 3000;
    this.host = options.host || "localhost";
    this.dashboardDir = options.dashboardDir || path.join(__dirname, "static");
    ...
    // Initialize TrackDown Manager
    this.trackdownManager = new TrackDownManager({
      config: this.config,
      projectsBasePath: options.projectsBasePath || process.cwd(),
  ...
  async start() {
    // Initialize TrackDown Manager
    try {
      await this.trackdownManager.initialize();
      console.log('✅ TrackDown Manager initialized');
    } catch (error) {
      console.warn('⚠️ TrackDown Manager initialization failed:', error.message);
    }
  ...
  async handleApiRequest(req, res, pathname) {
    ...
    switch (pathname) {
      ...
      // TrackDown API endpoints
      case "/api/trackdown/projects":
        await this.handleTrackDownProjectsRequest(res);
        break;

      case "/api/trackdown/tickets":
        if (req.method === "POST") {
          await this.handleTrackDownCreateTicket(req, res);
        } else {
          await this.handleTrackDownTicketsRequest(req, res);
        }
        break;

      case "/api/trackdown/analytics":
        await this.handleTrackDownAnalyticsRequest(res);
        break;

      case "/api/trackdown/sync":
        if (req.method === "POST") {
          await this.handleTrackDownSyncRequest(res);
        } else {
          this.sendErrorResponse(res, 405, "Method not allowed");
        }
        break;
      ...
      default: {
        // Check for project removal requests
        const projectRemovalMatch = pathname.match(/^\/api\/projects\/(.+)$/);
        if (projectRemovalMatch && req.method === "DELETE") {
          await this.handleProjectRemoval(req, res, projectRemovalMatch[1]);
          break;
        }
        
        // Check for TrackDown ticket operations
        const trackdownTicketMatch = pathname.match(/^\/api\/trackdown\/tickets\/([^\/]+)\/([^\/]+)$/);
        if (trackdownTicketMatch) {
          await this.handleTrackDownTicketOperation(req, res, trackdownTicketMatch[1], trackdownTicketMatch[2]);
          break;
        }
   ...
   */
  async handleProjectRemoval(req, res, projectName) {
    try {
      const projectDataDir = path.join(this.dataDir, projectName);
      ...
      await this.removeDirectoryRecursively(projectDataDir);
      
      console.log(`🗑️ Removed project from portfolio: ${projectName}`);
      
      this.sendJsonResponse(res, {
        success: true,
        message: `Project '${projectName}' removed from portfolio`,
        projectName: projectName,
        timestamp: new Date().toISOString()
      });
    } catch (error) {
      console.error(`❌ Error removing project ${projectName}:`, error);
      this.sendErrorResponse(res, 500, "Failed to remove project");
    }
  }

  /**
   * Remove directory recursively (safe deletion)
   */
  async removeDirectoryRecursively(dirPath) {
    if (!fs.existsSync(dirPath)) return;
    
    // Safety check - only remove directories within data dir
    if (!dirPath.startsWith(this.dataDir)) {
      throw new Error("Cannot remove directories outside of data directory");
    }
  ...
  async loadLatestProjectData(projectDir, projectName) {
    try {
      ...
      // Use toolchain from health data first, then fall back to detection
      const toolchain = healthData.toolchain || this.determineToolchain(activityData);
      ...
      const transformedProject = {
        ...
        hasTrackdown: healthData.hasTrackdown || activityData.hasTrackdown || false,
        git: {
  ...
  /**
   * Handle TrackDown projects request
   */
  async handleTrackDownProjectsRequest(res) {
    try {
      const projects = await this.trackdownManager.getProjects();
      this.sendJsonResponse(res, {
        success: true,
        data: projects,
        timestamp: new Date().toISOString()
      ...
      console.error("❌ Error fetching TrackDown projects:", error);
      this.sendErrorResponse(res, 500, "Failed to fetch projects");
    }
  }

  /**
   * Handle TrackDown tickets request
   */
  async handleTrackDownTicketsRequest(req, res) {
    try {
      ...
      const filters = {};
      if (query.projects) filters.projects = Array.isArray(query.projects) ? query.projects : [query.projects];
      ...
      if (query.epic) filters.epic = query.epic;
      if (query.sprint) filters.sprint = query.sprint;
      if (query.search) filters.search = query.search;
      
      const tickets = await this.trackdownManager.getTickets(filters);
      this.sendJsonResponse(res, {
        success: true,
        data: tickets,
        timestamp: new Date().toISOString()
      ...
      console.error("❌ Error fetching TrackDown tickets:", error);
      this.sendErrorResponse(res, 500, "Failed to fetch tickets");
    }
  }

  /**
   * Handle TrackDown analytics request
   */
  async handleTrackDownAnalyticsRequest(res) {
    try {
      const analytics = await this.trackdownManager.getAnalytics();
      this.sendJsonResponse(res, {
        success: true,
        data: analytics,
        timestamp: new Date().toISOString()
      ...
      console.error("❌ Error fetching TrackDown analytics:", error);
      this.sendErrorResponse(res, 500, "Failed to fetch analytics");
    }
  }

  /**
   * Handle TrackDown sync request
   */
  async handleTrackDownSyncRequest(res) {
    try {
      // Force cache refresh to sync data
      await this.trackdownManager.refreshCache();
      
      this.sendJsonResponse(res, {
        success: true,
        message: "TrackDown data synchronized successfully",
        timestamp: new Date().toISOString()
      });
    } catch (error) {
      console.error("❌ Error syncing TrackDown:", error);
      this.sendErrorResponse(res, 500, "Failed to sync TrackDown data");
    }
  }

  /**
   * Handle TrackDown create ticket request
   */
  async handleTrackDownCreateTicket(req, res) {
    try {
      const ticketData = await this.parseRequestBody(req);
      const newTicket = await this.trackdownManager.createTicket(ticketData.project, ticketData);
      this.sendJsonResponse(res, {
        success: true,
        data: newTicket,
      ...
      console.error("❌ Error creating TrackDown ticket:", error);
      this.sendErrorResponse(res, 500, error.message || "Failed to create ticket");
    }
  }

  /**
   * Handle TrackDown ticket operations (CRUD)
   */
  async handleTrackDownTicketOperation(req, res, projectName, ticketId) {
    try {
      switch (req.method) {
        case 'GET':
          const ticket = await this.trackdownManager.getTicket(projectName, ticketId);
          if (!ticket) {
            this.sendErrorResponse(res, 404, "Ticket not found");
            return;
          }
          this.sendJsonResponse(res, {
            success: true,
        ...
        case 'PUT':
          ...
          const updatedTicket = await this.trackdownManager.updateTicket(projectName, ticketId, updateData);
          this.sendJsonResponse(res, {
            success: true,
            data: updatedTicket,
        ...
        case 'DELETE':
          await this.trackdownManager.deleteTicket(projectName, ticketId);
          this.sendJsonResponse(res, {
            success: true,
      ...
      console.error("❌ Error handling TrackDown ticket operation:", error);
      this.sendErrorResponse(res, 500, error.message || "Failed to handle ticket operation");
    }
  }
}
