...
class PortfolioDashboard {
    ...
    updateDashboardHeader(title) {
        ...
        // Update Add Site button based on project selection
        this.updateAddSiteButton();
    ...
    updateAddSiteButton() {
        ...
        const addSiteDropdown = document.getElementById('addSiteDropdown');
        
        if (!addSiteBtn) return;
        
        if (this.selectedProject) {
            // Change to "Remove This Site" button
            addSiteBtn.innerHTML = `
                <i data-lucide="trash-2" class="h-4 w-4"></i>
                <span>Remove This Site</span>
            `;
            addSiteBtn.className = 'flex items-center space-x-2 px-3 py-2 text-sm text-red-600 hover:text-red-900 border border-red-300 rounded-md hover:bg-red-50';
            addSiteBtn.onclick = () => this.removeCurrentProject();
            ...
            // Change back to "Add Site" button
            addSiteBtn.innerHTML = `
                <i data-lucide="plus" class="h-4 w-4"></i>
                <span>Add Site</span>
            `;
            addSiteBtn.className = 'flex items-center space-x-2 px-3 py-2 text-sm text-blue-600 hover:text-blue-900 border border-blue-300 rounded-md hover:bg-blue-50';
            addSiteBtn.onclick = () => this.toggleAddSiteDropdown();
        }
    }
    ...
    async removeCurrentProject() {
        ...
        if (!confirm(`Are you sure you want to remove "${this.selectedProject}" from the portfolio?\n\nThis will only remove it from monitoring, not delete the actual project files.`)) {
            return;
        }

        try {
            console.log(`🗑️ Removing current project: ${this.selectedProject}`);
            
            const response = await fetch(`/api/projects/${encodeURIComponent(this.selectedProject)}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json'
                }
            ...
            // Show success message
            this.showNotification(`Project "${this.selectedProject}" removed from portfolio`, 'success');

            // Clear selection and return to portfolio view
            this.clearSelectedProject();
            ...
            console.error('❌ Failed to remove project:', error);
            this.showNotification(`Failed to remove project "${this.selectedProject}": ${error.message}`, 'error');
        }
    }
